...
export default function Savings({ state, setState, monthKey, monthData }) {
  ...
  return (
    <div className="space-y-4 pb-4">
      <h2 className="text-xl font-bold px-1">Savings</h2>
      ...
      <div className="card">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">
          Mark as Saved — {MONTH_NAMES[month]}
        </h3>
            ...
            <div className="flex-1">
              ...
              {carChecked && (
                <p className="text-[10px] text-emerald-500 font-semibold">
                  Saved on {new Date().toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                </p>
              )}
            </div>
            <span className={`text-sm mono ${carChecked ? "text-emerald-500 font-bold" : "font-semibold"}`}>
              {formatPKR(alloc.carFund)}
            </span>
          </label>
        )}
        ...
        {alloc.emergencyFund > 0 && (
          <label className="flex items-center gap-3 py-3 cursor-pointer">
            ...
            <div className="flex-1">
              <span className={`text-sm ${emChecked ? "line-through text-slate-400" : "font-medium"}`}>🛡️ Emergency Fund (15%)</span>
              {emChecked && (
                <p className="text-[10px] text-emerald-500 font-semibold">
                  Saved on {new Date().toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                </p>
            ...
            <span className={`text-sm mono ${emChecked ? "text-emerald-500 font-bold" : "font-semibold"}`}>
              {formatPKR(alloc.emergencyFund)}
            </span>
          </label>
        )}
        ...
        <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-800 flex justify-between text-sm font-bold">
          ...
          <span className="mono">{formatPKR(alloc.carFund + alloc.emergencyFund)}</span>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="flex justify-between text-sm mb-2">
          ...
          <span className="font-bold mono">{formatPKR(car.total)} / {formatPKR(SAVINGS_TARGETS.carFund.target)}</span>
        </div>
        <ProgressBar current={car.total} target={SAVINGS_TARGETS.carFund.target} color="blue" height="h-3" showPct={false} />
        <div className="flex justify-between text-xs text-slate-400 mt-2">
          <span>Remaining: {formatPKR(Math.max(SAVINGS_TARGETS.carFund.target - car.total, 0))}</span>
            ...
            {car.contributions.slice().reverse().slice(0, 6).map((c, i) => (
              <div key={i} className="flex justify-between text-xs py-1">
                <span className="text-slate-400">
                  {c.month} — {c.type === "extra" ? (c.description || "Extra") : "Monthly"}
                </span>
      ...
      <div className="card">
        ...
        <div className="flex justify-between text-sm mb-2">
          <span className="text-slate-500">Saved so far</span>
          <span className="font-bold mono">{formatPKR(emergency.total)} / {formatPKR(SAVINGS_TARGETS.emergencyFund.target)}</span>
        </div>
        <ProgressBar current={emergency.total} target={SAVINGS_TARGETS.emergencyFund.target} color="emerald" height="h-3" showPct={false} />
        <div className="flex justify-between text-xs text-slate-400 mt-2">
          <span>Remaining: {formatPKR(Math.max(SAVINGS_TARGETS.emergencyFund.target - emergency.total, 0))}</span>
        ...
        <div className="card">
          ...
          {monthData.extraSavings.map((e) => (
            <div key={e.id} className="flex items-center gap-3 py-2 border-b border-slate-50 dark:border-slate-800 last:border-0">
              ...
              <span className="text-sm font-bold mono text-emerald-500">+{formatPKR(e.amount)}</span>
              <button onClick={() => handleDeleteExtra(e.id)} className="text-slate-300 hover:text-red-500 text-lg">×</button>
            </div>
          ))}
        </div>
      )}
      ...
      {showAddExtra ? (
        ...
        <button onClick={() => setShowAddExtra(true)}
          className="w-full card text-center text-emerald-600 font-semibold text-sm py-3 active:bg-emerald-50">
          + Add Extra Saving
        </button>
      )}
    </div>
